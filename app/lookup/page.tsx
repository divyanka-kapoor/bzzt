  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      <nav className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
        <Link href="/" className="text-sm font-bold text-white hover:text-white/70 transition-colors">Bzzt</Link>
        <Link href="/dashboard" className="text-xs text-white/30 hover:text-white/60 transition-colors">Operator Dashboard →</Link>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <form onSubmit={lookup} className="flex gap-2 mb-8">
          <input
            ...
            placeholder="Enter any city — Jakarta, Lagos, São Paulo…"
            className="flex-1 bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/25 focus:outline-none focus:border-white/25 text-sm"
          />
          <button type="submit" disabled={loading || !query.trim()}
            className="px-5 py-3 bg-white text-black text-sm font-semibold rounded-xl hover:bg-white/90 transition disabled:opacity-40">
            {loading ? '…' : 'Check'}
          </button>
        </form>

        {error && (
          <p className="text-center text-sm text-white/40 mb-6">{error}</p>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-sm text-white/40">{result.country}</p>
              </div>
              {topRisk && (
            ...
            <div className="rounded-xl border border-white/[0.06] p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {result.qualityChecks.map(q => (
                  <span key={q.name} className={`text-[9px] px-2 py-0.5 rounded border font-mono ${
                    q.passed
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5 text-center">
              ...
              <p className="text-xs text-white/40 mb-4">We monitor 24/7 and only alert when climate signals shift meaningfully.</p>
              <Link href="/" className="inline-block px-5 py-2.5 bg-white text-black text-sm font-semibold rounded-lg hover:bg-white/90 transition">
        ...
        {!result && !loading && (
          <div className="mt-12 text-center space-y-4">
            ...
            <p className="text-[11px] text-white/20">◉ dengue (circle) · ◆ malaria (diamond)</p>
          </div>
        )}
      </div>
    </main>
